...
EXPORT_DIR = ROOT / "translations" / "export"
...
def harvested(export_dir: Path) -> set[str]:
    """Key names the game's own string cache produced (translations/export/cache_*.lua).

    Mining the installed mods only finds keys a mod happens to reference, and the exporter only
    answers for names somebody wrote down. The cache points the other way: it holds the key of every
    string a session resolved, so whatever it names is a key the game really has - the abilities of a
    class added after this list was written, the enemy families, the mission names. Written by
    exporter.harvest_cache() while the game runs.
    """
    found = set()
    for path in sorted(export_dir.glob("cache_*.lua")):
        found.update(KEY_RE.findall(path.read_text(encoding="utf-8", errors="ignore")))
    return found
...
def main() -> int:
    ...
    # Keys the game named itself while the player browsed: real keys, including the ones no amount
    # of pattern guessing reaches (see harvested()). They are added as their own block so the file
    # says where they came from.
    observed = harvested(EXPORT_DIR)
    if observed:
        groups.append(("named by the game's string cache", sorted(observed)))
